...
3. Inspect both anchors before spending on video. Reject identity/product drift or an endpoint that implies an unsupported result. Prefer direct image APIs with verified rights, capacity and approved costs; supported subscription-backed operations are also possible. Do not hide billed operations or spend without approval.
...
6. Assemble selected A-roll/B-roll, captions, sound and CTA. Preserve takes, trims, subtitle timings and the edit decision list. Use authorized local, subscription, owned or approved paid audio routes, with explicit costs and rights. There is no video-only paid-API cap.
7. Decode and review the entire rendered ad against the selected reference for natural delivery, coherent motion, product accuracy, readable captions, pacing, narrative, CTA, audio and ad-to-page consistency. Functionally test the destination and purchase path as well. Technical pass, creative pass and owner acceptance are separate.
...
The developer can implement this in custom code with a node inspection/editing surface. The historical recommendation to make the visual editor optional is a past architecture opinion, not a reason to erase the requested node-based workflows. Current custom-build preference and full-system scope govern. Preserve inspectable lineage even when a newer model, skill or whole workflow combines/removes obsolete generation stages; the historical graph is not a permanent agent architecture.
...
This creative demonstration is one part of the completed multi-brand private-cloud ecommerce engine, not final acceptance of a standalone video tool. Create the referenced capabilities inside the owned engine rather than supplying SaaS links. Prefer direct image AND video APIs; an aggregator is justified only where its full workflow/skill improves the rendered output, through an owned replaceable adapter. Codex AND Claude must power development and the deployed engine through verified official supported access, capacity and runtime terms.

Connect original product-specific finished ads to complete Shopify stores, live budgeted campaigns, verified purchase tracking and the included sourcing-to-post-purchase/accounting operations described in the [semantic workflow](semantic-workflows.md). Record spend, results and decisions and produce the next measured iteration. **60 live product tests per month** is the operating and final-acceptance requirement, with completed research/economics/offer and a functioning checkout destination for each test. Multiple takes or ad variants for one product, diagrams, simulations and replay demonstrations are not additional product tests. Preagree measurement windows and obtain separate operating-spend approval. Initial work samples do not replace full-system commissioning; demos, scaffolds, disconnected automation and partial builds are not acceptable final delivery.
