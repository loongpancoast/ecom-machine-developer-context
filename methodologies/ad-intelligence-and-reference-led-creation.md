...
Find products worth testing, show the relevant ads and landing pages, explain which patterns are worth trying, then produce ORIGINAL product-specific finished ads and functioning Shopify stores/pages. Launch live budgeted campaigns with verified purchase tracking and return measured results to the same records so the next batch uses what was learned. These are connected capabilities to CREATE inside the full owner-controlled multi-brand private-cloud autonomous ecommerce engine, not separate dashboards or a bundle of SaaS links. Research, economics, offers, stores, campaigns and sourcing-through-post-purchase/accounting operations are all commissioned scope; see the [operational chain](semantic-workflows.md).
...
Study and test a supported complete workflow before rebuilding its steps. It may outperform a custom chain. Prefer direct image AND video APIs; use an aggregator only when its full workflow/skill improves the rendered result, through an owned replaceable adapter. Keep product and identity references consistent, inspect the finished result, and use additional editing only when it improves the ad. Website generation must still be tested as a functioning Shopify store/page with native Liquid and a working purchase path, not accepted because it produced HTML.
...
2. Scrape/capture permitted ads and destination pages, then break down the hook, angle, script, pacing, narrative, shot rhythm, product reveal, demonstration, proof, offer and CTA. Inspect actual decoded media, not only transcripts or thumbnails. For a landing page, capture section order, imagery, offer, objections, proof, mobile behavior and purchase path. Keep the ad-to-page connection.
...
5. Show the reference and actually decoded/rendered output together for visual and functional review. Check visual quality, product accuracy, pacing and ad/page consistency, then test cart/checkout and conversion events, including consent and browser/server deduplication. Retain a control and state what each variant changes.
6. Launch under approved account permissions and budgets. Join recorded spend, purchase/results data and decisions to the exact output versions, then create and measure the next iteration. A polished comparison without live execution and feedback is not completed-system acceptance.

These are required information and output relationships, not a fixed agent count or permanent sequence of prompts. Better models, providers, skills and whole workflows can replace, combine or remove obsolete stages while preserving data, business context, history, source assets and results. Compare actual quality, cost, speed and reliability before adoption, with versioning and demonstrated restore.
...
## Demonstration, commissioning and cost boundary

Use an agreed product and permitted references for the bounded workflow demonstration. Show the visual research dossier, the reference-to-output map, a finished ad and functioning Shopify page, then a revised brief or variant informed by joined performance data. If only historical data is available for an initial work sample, label the replay and do not invent a new live result. Initial samples, replays and upgrade demonstrations are distinct from final acceptance. Preserve the same evidence library through the model/workflow replacement test in [agent-led execution](agent-led-execution.md#upgrade-demonstration).

Final delivery is the completed full commissioned system, not a demo, scaffold, disconnected automation or partial build. It must operate **60 live product tests per month**. Each qualifying test has completed research/economics/offer, finished ads, a functioning checkout destination, a live budgeted campaign, verified tracking and recorded spend, results and decisions. Research rows, creative variants, simulations and replays do not count. Preagree measurement windows and separately approve operating spend. This is a real execution/measurement requirement, not a guarantee of profitable winners. Milestones stage the complete delivery, not a reduced core with the rest deferred.

These services are capability and output-quality references to implement within the owned engine, not a requirement to buy or integrate all of them, reproduce their proprietary datasets, or clone every vendor feature. Deliver the required connected capabilities within the agreed fee. Use supported integration routes and authorized data access; identify owner/KYC/legal approvals and access dependencies without turning them into exclusions or a permanent manual substitute for the commissioned automation.

An MCP connection does not make a paid API or generation free. Bundled workflows can charge separately for images, audio, research and video. The former video-only paid-API cap is superseded; direct image AND video APIs are preferred, and an aggregator must justify its full-workflow/skill quality benefit. Verify account-specific allowances, rights, costs and capacity; any operating spend or new subscription needs approval. Codex AND Claude must power development and the deployed engine through verified official supported access and runtime terms, not an assumed subscription-to-API entitlement. Keep reference selection, generation services and learned records replaceable as models improve.
